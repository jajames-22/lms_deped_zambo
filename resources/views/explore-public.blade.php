...
<body class="bg-gray-50 text-gray-800 relative">
    ...
    <main class="pt-24 md:pt-32 pb-24">
        ...
        <div id="main-explore-content"
            class="max-w-7xl mx-auto space-y-10 pb-24 relative px-4 sm:px-6 transition-opacity duration-300">

            {{-- BACK BUTTON --}}
            <button onclick="navigateBack()"
                class="cursor-pointer flex items-center w-fit text-gray-500 hover:text-[#a52a2a] font-bold transition-colors group px-2 py-1 rounded-lg hover:bg-red-50 relative z-10 mb-2">
                <i class="fas fa-arrow-left mr-2 group-hover:-translate-x-1 transition-transform"></i> <span
            ...
            <div class="px-2 flex flex-col md:flex-row md:items-end justify-between gap-6 mb-4">
                <div>
                    ...
                    <p class="text-gray-500 mt-2 text-base md:text-lg">Explore open-access learning resources from DepEd
                        Zamboanga.</p>
                </div>
                ...
                <div class="relative w-full md:w-96 group shrink-0">
                    ...
                    <button onclick="executePublicSearch()"
                        class="absolute right-1.5 top-1/2 -translate-y-1/2 bg-[#a52a2a] hover:bg-red-800 text-white px-4 py-2 rounded-lg font-bold shadow-sm transition-all text-xs active:scale-95">
                        Search
                    </button>
                </div>
            </div>
            ...
            @if($featuredMaterials->isNotEmpty())
                <div class="relative w-full h-80 md:h-[450px] rounded-2xl overflow-hidden shadow-xl group"
                    ...
                    <div class="flex transition-transform duration-700 ease-in-out h-full w-full" id="carousel-track">
                        @foreach($featuredMaterials as $index => $material)
                            <div class="w-full h-full flex-shrink-0 relative cursor-pointer material-search-item"
                                data-id="{{ $material->id }}" data-title="{{ $material->title }}"
                                ...
                                <div class="absolute inset-0 z-20 flex flex-col justify-end p-8 md:p-16 w-full md:w-3/4">
                                    <span
                                        class="px-3 py-1 bg-[#a52a2a] text-white text-[10px] font-black uppercase tracking-[0.2em] rounded-md w-max mb-4 shadow-md">Featured</span>
                                    <h1
                                        class="text-4xl md:text-5xl lg:text-6xl font-black text-white mb-4 leading-tight drop-shadow-lg line-clamp-2">
                                        {{ $material->title }}</h1>

                                    <p
                                        class="text-white font-bold text-xs md:text-sm uppercase tracking-widest mb-2 flex items-center gap-1.5 drop-shadow-md">
                                        <i class="fas fa-chalkboard-user"></i>
                                        {{ $material->instructor->first_name ?? 'Instructor' }}
                                        {{ $material->instructor->last_name ?? '' }}
                                    </p>

                                    <p class="text-gray-100 text-sm md:text-lg mb-4 line-clamp-2 max-w-2xl drop-shadow-md">
                                        {{ $material->description }}</p>

                                    <div class="flex items-center gap-4">
                                        <button
                                            class="bg-white text-black hover:bg-gray-200 font-bold py-3 px-8 rounded-lg transition-all flex items-center gap-2 shadow-xl">
                    ...
                    @if($featuredMaterials->count() > 1)
                        <button onclick="window.moveCarousel(-1)"
                            class="absolute left-4 top-1/2 -translate-y-1/2 z-30 w-12 h-12 flex items-center justify-center bg-black/40 hover:bg-[#a52a2a] text-white rounded-full backdrop-blur-md transition-all opacity-0 group-hover:opacity-100 shadow-lg"><i
                                class="fas fa-chevron-left"></i></button>
                        <button onclick="window.moveCarousel(1)"
                            class="absolute right-4 top-1/2 -translate-y-1/2 z-30 w-12 h-12 flex items-center justify-center bg-black/40 hover:bg-[#a52a2a] text-white rounded-full backdrop-blur-md transition-all opacity-0 group-hover:opacity-100 shadow-lg"><i
                                class="fas fa-chevron-right"></i></button>
                        <div class="absolute bottom-6 left-1/2 -translate-x-1/2 z-10 flex gap-2">
                            @foreach($featuredMaterials as $index => $material)
                                <button onclick="window.goToSlide({{ $index }})"
                                    class="carousel-dot h-2.5 rounded-full transition-all duration-300 {{ $index === 0 ? 'bg-[#a52a2a] w-8' : 'bg-white/50 hover:bg-white w-2.5' }}"></button>
                            @endforeach
                        </div>
                    @endif
                </div>
                        ...
                        <div class="flex overflow-x-auto no-scrollbar gap-6 pb-6 snap-x px-2">
                            @foreach($section->materials as $material)
                                <div class="w-72 flex-none snap-start group bg-white border border-gray-200 hover:border-[#a52a2a]/30 rounded-xl overflow-hidden shadow-sm hover:shadow-md transition-all duration-300 flex flex-col cursor-pointer material-search-item"
                                    ...
                                    <div class="p-5">
                                        <h3
                                            class="font-bold text-gray-900 line-clamp-1 group-hover:text-[#a52a2a] transition-colors duration-300">
                                            {{ $material->title }}</h3>
                                        <p
                                            class="text-[10px] text-[#a52a2a] font-bold uppercase tracking-wider mt-1.5 truncate flex items-center gap-1.5">
                                            <i class="fas fa-chalkboard-user"></i>
                                            {{ $material->instructor->first_name ?? 'Instructor' }}
                                            {{ $material->instructor->last_name ?? '' }}
                                        </p>
                ...
                <div class="flex overflow-x-auto no-scrollbar pb-4 px-2">
                    @foreach($popularMaterials as $index => $material)
                        <div class="flex-none flex items-center gap-4 group cursor-pointer material-search-item"
                            ...
                            onclick="window.location.href = '{{ route('explore.materials.show', $material->id) }}';">
                            <span
                                class="text-7xl md:text-8xl font-black text-gray-200 group-hover:text-[#a52a2a]/20 transition-colors italic leading-none">{{ $index + 1 }}</span>
                            <div class="h-64 rounded-xl overflow-hidden -translate-x-6 shadow-lg relative">
                                <img src="{{ $material->thumbnail ? asset('storage/' . $material->thumbnail) : 'https://images.unsplash.com/photo-1517694712202-14dd9538aa97?q=80&w=300' }}"
                                    class="w-full h-full object-cover group-hover:scale-110 transition-transform duration-500">
                                <div
                                    class="absolute inset-0 bg-gradient-to-t from-black/90 via-transparent to-transparent p-4 flex flex-col justify-end">
                                    <p class="text-white font-bold text-sm leading-tight line-clamp-2">
                                        {{ $material->title }}</p>
                                    <p
                                        class="text-gray-300 text-[10px] font-medium mt-1.5 truncate flex items-center gap-1.5">
                                        <i class="fas fa-chalkboard-user text-gray-400"></i>
                                        {{ $material->instructor->first_name ?? 'Instructor' }}
                                        {{ $material->instructor->last_name ?? '' }}
                                    </p>
                                    <p class="text-gray-400 text-[10px] mt-1"><i
                                            class="fas fa-eye mr-1"></i>{{ number_format($material->views) }} views</p>
                                </div>
                            </div>
                        </div>
                    @endforeach
            ...
            {{-- 4. GUEST CTA --}}
            <section
                class="mt-8 bg-[#a52a2a]/5 border border-[#a52a2a]/20 rounded-2xl p-6 md:p-8 text-center max-w-4xl mx-auto">
                <i class="fas fa-school text-3xl text-[#a52a2a] mb-3"></i>
                <h2 class="text-xl md:text-2xl font-extrabold text-gray-900 tracking-tight mb-2">Want to see materials
                    specific to your school?</h2>
                <p class="text-gray-600 max-w-xl mx-auto mb-6 text-sm md:text-base">Create a free student account today
                    to access tailored modules, track your learning progress, and earn certificates from instructors at
                    your registered school.</p>
                <div class="flex flex-col sm:flex-row justify-center gap-3">
                    <a href="{{ route('register') }}"
                        class="px-6 py-2.5 bg-[#a52a2a] hover:bg-red-800 text-white text-sm font-bold rounded-lg shadow-md transition-all">Create
                        Account</a>
                    <a href="{{ route('login') }}"
                        class="px-6 py-2.5 bg-white text-[#a52a2a] border border-[#a52a2a]/30 hover:bg-gray-50 text-sm font-bold rounded-lg shadow-sm transition-all">I
        ...
        <div id="filtered-explore-content"
            class="max-w-7xl mx-auto hidden px-4 sm:px-6 transition-opacity duration-300">
            <div class="flex flex-col md:flex-row md:items-center gap-4 mb-10">
                <button onclick="resetExploreView()"
                    class="h-12 w-12 flex items-center justify-center rounded-full bg-white border border-gray-200 text-gray-500 hover:text-[#a52a2a] transition shadow-sm group shrink-0">
            ...
            <div id="filtered-materials-grid"
