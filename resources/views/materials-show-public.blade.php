            ...
            foreach($section->items as $item) {
                if(!empty($item->media_url)) {
                    ...
                    $resources->push((object)[
                        ...
                        'name' => $name,
                        'size' => $size,
                        'url' => $mediaUrl // URL added for download
                    ]);
                }
            }
        }
        $resources = $resources->unique('name')->values();
    @endphp

    {{-- WRAPPER FOR ANIMATION --}}
    <div id="page-wrapper" class="min-h-screen flex flex-col animate-slide-in">

        {{-- CLEAN HEADER (Guest Version) --}}
        <header class="bg-white border-b border-gray-200 sticky top-0 z-50 shadow-sm">
            <div class="max-w-6xl mx-auto px-4 md:px-8 h-16 flex items-center justify-between relative">
                <button onclick="navigateBack()" class="flex items-center text-gray-500 hover:text-[#a52a2a] font-bold transition-colors group px-2 py-1 rounded-lg hover:bg-red-50 relative z-10">
        ...
        <main class="flex-1 max-w-5xl mx-auto w-full py-8 px-4 sm:px-6 lg:px-8 pb-24">
            ...
            <div class="bg-white rounded-3xl shadow-sm border border-gray-100 p-6 md:p-8 mb-10 flex flex-col md:flex-row gap-8 lg:gap-12">
                ...
                <div class="w-full md:w-1/2 lg:w-7/12 flex flex-col">
                    <div class="flex flex-wrap gap-2 mb-4">
                        ...
                        @empty
                            <span class="px-3 py-1 bg-gray-100 text-gray-500 border border-gray-200 text-[10px] font-black uppercase tracking-wider rounded-md">General</span>
                        @endforelse
                    </div>

                    <h1 class="text-3xl md:text-4xl lg:text-5xl font-black text-gray-900 mb-4 leading-tight">{{ $material->title }}</h1>
                    <p class="text-gray-600 text-sm md:text-base leading-relaxed mb-6 line-clamp-4">{{ $material->description }}</p>
                    ...
                    <div class="flex flex-wrap items-center gap-6 py-4 border-t border-gray-100 mt-auto">
                        <div class="flex items-center gap-3">
                            ...
                            <div>
                                ...
                                <p class="text-sm font-bold text-gray-900">{{ $material->instructor->first_name ?? 'Unknown' }} {{ $material->instructor->last_name ?? '' }}</p>
                            </div>
                        </div>
                    ...
                    <div id="action-buttons-container" class="flex flex-wrap items-center gap-3">
                        <a href="{{ route('login') }}" class="flex w-full sm:w-auto px-8 py-3.5 bg-[#a52a2a] text-white font-bold rounded-xl hover:bg-red-800 transition shadow-lg shadow-[#a52a2a]/20 items-center justify-center gap-2">
                            <i class="fas fa-lock text-lg"></i> Login to Enroll
                        </a>
                    </div>
                </div>
            </div>
            ...
            <div class="bg-white rounded-3xl shadow-sm border border-gray-100 p-2 md:p-4">
                @forelse($timeline as $index => $section)
                    <div class="lesson-item p-4 rounded-2xl transition-colors border border-transparent flex items-start gap-4 opacity-70 cursor-not-allowed group">
                        ...
                        <div class="flex-1 min-w-0 pt-1.5">
                            <h4 class="font-bold text-gray-900 text-lg">{{ $section->title }}</h4>
                        ...
                        <div class="pt-3 pl-4">
                            <i class="lesson-status-icon fas fa-lock text-gray-300 tooltip" title="Login to unlock"></i>
                        </div>
                    </div>
                @empty
                ...
                <div class="bg-white rounded-3xl shadow-sm border border-gray-100 p-2 md:p-4 grid grid-cols-1 sm:grid-cols-2 gap-2">
                    @foreach($resources as $res)
                        ...
                        <a href="{{ $res->url }}" download target="_blank" class="flex items-center gap-4 p-3 rounded-2xl hover:bg-gray-50 border border-transparent hover:border-gray-100 transition group cursor-pointer">
                            ...
                            <div class="h-10 w-10 shrink-0 rounded-full flex items-center justify-center text-[#a52a2a] opacity-0 group-hover:opacity-100 transition bg-red-50 hover:bg-red-100" title="Download Resource">
