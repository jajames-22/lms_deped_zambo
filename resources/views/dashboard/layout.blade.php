...
<body class="bg-gray-50 font-sans text-gray-900 h-screen overflow-hidden">
    ...
    <div class="flex h-full"> 
        ...
        <aside id="sidebar" class="fixed inset-y-0 left-0 w-64 bg-white border-r border-gray-200 z-50 transform -translate-x-full md:translate-x-0 md:relative sidebar-transition flex flex-col h-full">
            <div class="px-6 py-8 flex items-center justify-between lg:justify-center shrink-0">
                <div class="flex-shrink-0">
                    <a href="{{ url('/dashboard') }}">
                        <img src="{{ asset('storage/images/lms-logo-red.png') }}" class="w-48 h-auto object-contain block" alt="LMS Logo">
                    </a>
                </div>
            ...
            <div class="border-t border-gray-100 shrink-0">
                <button onclick="toggleLogoutModal()" class="w-full flex items-center px-4 py-3 text-gray-600 hover:bg-red-50 hover:text-red-600 transition font-medium">
    ...
    <div id="logoutModal" class="fixed inset-0 z-[60] hidden">
        ...
        <div class="relative flex items-center justify-center min-h-screen p-4">
            <div class="bg-white rounded-2xl shadow-2xl max-w-sm w-full p-6 text-center transform transition-all border border-gray-100">
                <div class="w-16 h-16 bg-[#a52a2a]/10 text-[#a52a2a] rounded-full flex items-center justify-center mx-auto mb-4">
                    <i class="fas fa-sign-out-alt text-2xl"></i>
                </div>
                <h3 class="text-xl font-bold text-gray-900 mb-2">Confirm Logout</h3>
                <p class="text-gray-500 mb-6 text-sm">Are you sure you want to end your session? Your progress will be saved.</p>
                <div class="flex space-x-3">
                    <button onclick="toggleLogoutModal()" class="flex-1 px-4 py-2.5 border border-gray-200 text-gray-600 font-semibold rounded-xl hover:bg-gray-50 transition text-sm">
                        Cancel
                    </button>
                    <form method="POST" action="{{ route('logout') }}" class="flex-1">
                        @csrf
                        <button type="submit" class="w-full px-4 py-2.5 bg-[#a52a2a] text-white font-semibold rounded-xl hover:opacity-90 transition shadow-lg shadow-[#a52a2a]/30 text-sm">
                            Yes, Logout
                        </button>
                    </form>
                </div>
