<div class="space-y-6 relative animate-float-in">
    <div class="flex flex-col md:flex-row md:items-center justify-between gap-4">
        ...
        <div class="flex-shrink-0 flex flex-wrap items-center gap-2 relative">
            <button id="bulkDeleteBtn" onclick="confirmDeleteStudent()"
            ...
            <table class="w-full text-left border-collapse" id="studentsTable">
                ...
                <tbody class="divide-y divide-gray-50">
                    @forelse($students as $student)
                        <tr class="hover:bg-gray-50/50 transition student-row"
                            data-status="{{ strtolower($student->status ?? 'pending') }}">
                            <td class="px-4 py-2.5 text-center">
                                <input type="checkbox" value="{{ $student->id }}"
                                    class="student-checkbox rounded border-gray-300 text-[#a52a2a] focus:ring-[#a52a2a] cursor-pointer">
                            </td>
                            <td class="px-4 py-2.5">
                                <div
                                    class="w-10 h-10 mx-auto rounded-full bg-green-50 border border-green-100 overflow-hidden flex items-center justify-center shadow-sm text-green-600 font-bold text-xs">
                            ...
                            <td class="px-4 py-2.5">
                                @if($student->grade_level)
                                    <span
                                        class="px-2 py-1 bg-purple-50 text-purple-700 border-purple-200 text-[10px] font-bold rounded-md border uppercase tracking-tighter">
                                        {{ $student->grade_level }}
                                    </span>
                                @else
                            ...
                            <td class="px-4 py-2.5 text-center">
                                <div class="flex items-center justify-center gap-1">
                                    <button
                                        onclick="loadPartial('{{ route('students.edit', $student->id) }}', document.getElementById('nav-students-btn'))"
...
<div id="exportModal"
    ...
    <div class="bg-white rounded-3xl shadow-2xl w-full max-w-md p-6 transform scale-95 transition-transform duration-300 border border-gray-100"
        ...
        <form action="{{ route('students.report') }}" method="GET" target="_blank">
            <div class="space-y-3 mb-6">
                <label class="flex items-center gap-3 p-3 border border-gray-200 rounded-xl cursor-pointer hover:bg-gray-50 transition-colors">
                    <input type="radio" id="export_status_all_students" name="status_type" value="all" checked class="w-5 h-5 text-[#a52a2a] border-gray-300 focus:ring-[#a52a2a]">
                    <span class="text-gray-700 font-bold">All Students</span>
                </label>
                <label class="flex items-center gap-3 p-3 border border-gray-200 rounded-xl cursor-pointer hover:bg-green-50 transition-colors">
                    <input type="checkbox" name="statuses[]" value="verified" class="export-status-cb-student w-5 h-5 text-green-600 rounded border-gray-300 focus:ring-green-600">
                    <span class="text-green-700 font-bold">Verified</span>
                </label>
                <label class="flex items-center gap-3 p-3 border border-gray-200 rounded-xl cursor-pointer hover:bg-amber-50 transition-colors">
                    <input type="checkbox" name="statuses[]" value="pending" class="export-status-cb-student w-5 h-5 text-amber-600 rounded border-gray-300 focus:ring-amber-600">
                    <span class="text-amber-700 font-bold">Pending</span>
                </label>
                <label class="flex items-center gap-3 p-3 border border-gray-200 rounded-xl cursor-pointer hover:bg-red-50 transition-colors">
                    <input type="checkbox" name="statuses[]" value="suspended" class="export-status-cb-student w-5 h-5 text-red-600 rounded border-gray-300 focus:ring-red-600">
