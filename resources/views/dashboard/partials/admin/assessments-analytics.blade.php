            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                ...
                <div class="bg-gradient-to-br from-gray-900 to-gray-800 p-5 rounded-2xl shadow-sm text-white flex flex-col justify-center lg:col-span-1">
                    <div class="flex items-center gap-1.5 mb-1 relative group w-fit cursor-help">
                       ...
                       <div class="absolute bottom-full left-0 mb-2 w-72 p-3 bg-white text-gray-700 text-[11px] leading-relaxed rounded-xl shadow-xl opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-200 z-50 border border-gray-100 font-normal tracking-normal pointer-events-none">
                            <strong class="text-gray-900 block mb-1">Mean Percentage Score</strong>
                            The average score of all the completed students. The standard target for mastery is usually 75%.
                ...
                <div class="bg-white p-5 rounded-2xl shadow-sm border border-gray-100 flex flex-col justify-center">
                    ...
                    <p class="text-2xl font-black text-blue-600">
                        {{ $proficientCount }} <span class="text-sm font-bold text-gray-400 tracking-wide uppercase">/ {{ $completedCount }} Completed</span>
                    </p>
                    <p class="text-[10px] text-gray-500 mt-1 leading-tight"><strong>{{ $proficiencyRate ?? 0 }}%</strong> met the 75% standard.</p>
                </div>
                ...
                <div class="bg-white p-6 md:p-8 rounded-2xl shadow-sm border border-gray-100 flex flex-col">
                    <div class="flex flex-col md:flex-row justify-between items-start mb-6 gap-4">
                        <div>
                            <div class="flex items-center gap-1.5 mb-1 relative group w-fit cursor-help">
                                ...
                                <div class="absolute bottom-full left-0 mb-2 w-72 p-4 bg-gray-900 text-gray-300 text-[11px] leading-relaxed rounded-xl shadow-xl opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-200 z-50 font-normal normal-case tracking-normal pointer-events-none">
                                    ...
                                    This chart maps the frequency of student scores directly into their corresponding mastery categories, providing an immediate visual curve of class performance. Only completed tests are shown.
                                    <div class="absolute top-full left-6 border-4 border-transparent border-t-gray-900"></div>
                                </div>
                            </div>
                            <p class="text-xs text-gray-500">Frequency of completed student scores mapped to standard proficiency levels.</p>
                        </div>
                        <div class="bg-blue-50 text-blue-700 px-4 py-2 rounded-xl text-sm font-bold border border-blue-100 whitespace-nowrap shadow-sm">
                            <i class="fas fa-users mr-1"></i> {{ number_format($completedCount ?? 0) }} Completed Takers
                        </div>
                    </div>
                                    ...
                                    @forelse($competencies ?? [] as $cat)
                                        <tr class="hover:bg-gray-50/50 transition">
                                            ...
                                            <td class="px-6 py-4 text-center">
                                                ...
                                                @else
                                                    <span class="px-2.5 py-1 rounded-md text-xs font-bold {{ ($cat->mps ?? 0) >= 75 ? 'text-green-700 bg-green-100' : (($cat->mps ?? 0) < 40 ? 'text-red-700 bg-red-100' : 'text-amber-700 bg-amber-100') }}">
                                                        {{ $cat->mps ?? 0 }}%
                                                    </span>
                                                @endif
                                            </td>
        ...
        <section id="ui-item-analysis" class="scroll-mt-20 mt-8">
            ...
            <div class="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
                <div class="p-6 border-b border-gray-100">
                    ...
                    <p class="text-xs text-gray-400">Shows how difficult each question was and which answers the completed students chose.</p>
                </div>
...
<div id="exportModal" class="fixed inset-0 bg-gray-900/50 backdrop-blur-sm z-[100] hidden flex items-center justify-center opacity-0 transition-opacity duration-300">
    <div class="bg-white rounded-2xl shadow-xl w-full max-w-md p-6 transform scale-95 transition-transform duration-300" id="exportModalContent">
        <div class="flex justify-between items-center mb-5">
            ...
            <button onclick="toggleExportModal()" class="text-gray-400 hover:text-gray-600 border-0 bg-transparent cursor-pointer"><i class="fas fa-times text-lg"></i></button>
        </div>
        ...
        <form action="{{ route('dashboard.assessments.export', $assessment->id) }}" method="GET" target="_blank">
            ...
            <div class="flex gap-3">
                <button type="button" onclick="toggleExportModal()" class="flex-1 bg-gray-100 hover:bg-gray-200 text-gray-800 py-2.5 rounded-xl font-medium border-0 transition-colors cursor-pointer">Cancel</button>
                <button type="submit" name="action" value="print" onclick="setTimeout(toggleExportModal, 500)" class="flex-1 bg-gray-800 hover:bg-gray-900 text-white py-2.5 rounded-xl font-medium border-0 transition-colors flex items-center justify-center gap-2 cursor-pointer"><i class="fas fa-print"></i> Print</button>
                <button type="submit" name="action" value="pdf" onclick="setTimeout(toggleExportModal, 500)" class="flex-1 bg-blue-600 hover:bg-blue-700 text-white py-2.5 rounded-xl font-medium border-0 transition-colors flex items-center justify-center gap-2 cursor-pointer"><i class="fas fa-file-pdf"></i> Download</button>
            </div>
        </form>
    </div>
</div>
