<div class="space-y-6 relative">
    <div class="flex flex-col md:flex-row md:items-center justify-between gap-4">
        ...
        <button onclick="loadPartial('{{ route('dashboard.schools.create') }}', document.getElementById('nav-schools-btn'))" 
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
        <div class="bg-white border border-gray-100 p-5 rounded-2xl shadow-sm flex items-center justify-between">
            ...
            <div class="w-10 h-10 bg-blue-50 text-blue-600 rounded-lg flex items-center justify-center"><i class="fas fa-school text-lg"></i></div>
        </div>
        </div>
                ...
                <tbody class="divide-y divide-gray-50">
                    @forelse($schools as $school)
                        <tr class="hover:bg-gray-50/50 transition">
                            <td class="px-6 py-4">
                                <div class="w-12 h-12 rounded-lg bg-gray-100 border border-gray-200 overflow-hidden flex items-center justify-center shadow-sm">
                                    @if($school->logo)
                            ...
                            <td class="px-6 py-4">
                                ...
                                <p class="text-xs text-gray-500 mt-1 max-w-[250px] truncate" title="{{ $school->address }}">
                                    <i class="fas fa-map-marker-alt text-[10px] mr-1"></i> {{ $school->address ?? 'No address provided' }}
                                </p>
                            </td>
                            ...
                            <td class="px-6 py-4">
                                ...
                                @endphp
                                <span class="px-2 py-1 {{ $style }} text-[10px] font-bold rounded-md border uppercase tracking-tighter">
                            ...
                            <td class="px-6 py-4">
                                <div class="flex flex-col">
                                    <span class="text-sm font-semibold text-gray-700">{{ $school->district->name ?? 'N/A' }}</span>
                            ...
                            <td class="px-6 py-4 text-center">
                                <div class="flex items-center justify-center gap-1">
                                    <button class="w-8 h-8 flex items-center justify-center text-gray-400 hover:text-blue-600 hover:bg-blue-50 rounded-lg transition shadow-none" title="Edit">
                                        <i class="fas fa-edit text-sm"></i>
                                    </button>
                                    <button class="w-8 h-8 flex items-center justify-center text-gray-400 hover:text-red-600 hover:bg-red-50 rounded-lg transition shadow-none" title="Delete">
