...
@php
    ...
    $rubricData = [];
    $passingRate = 75;
    $criteriaTitle = 'Custom Criteria';

    // You must pass $evaluationCriteria from your MaterialController's evaluateMaterial method based on request('criteria_id')
    if (isset($evaluationCriteria)) {
        $criteriaTitle = $evaluationCriteria->title ?? 'Custom Criteria';
        $passingRate = $evaluationCriteria->passing_rate ?? 75;

        // Parse JSON if needed
        $parsed = is_string($evaluationCriteria->rubric) ? json_decode($evaluationCriteria->rubric, true) : $evaluationCriteria->rubric;

        // Account for both the legacy schema and the new rubric array schema
        if (is_array($parsed) && isset($parsed['rubric'])) {
...
<body
    class="bg-gray-50 font-sans text-gray-900 h-screen overflow-hidden flex flex-col selection:bg-red-900 selection:text-white">
    ...
    <header
        ...
        <div class="flex-1 flex flex-col items-center justify-center">
            <div class="flex items-center gap-2">
                <span
                    class="px-2 py-0.5 bg-blue-100 text-blue-700 text-[9px] font-black uppercase tracking-widest rounded">Admin
        ...
        <div class="w-1/4 flex items-center justify-end gap-4 shrink-0">
            <button onclick="toggleMobileEval()"
                class="lg:hidden px-4 py-2 bg-red-900 text-white text-xs font-bold rounded-lg shadow-sm">
    ...
    <div class="flex flex-1 overflow-hidden relative z-10">
        ...
        <main class="flex-1 flex flex-col min-w-0 bg-gray-50 h-full relative z-10 border-r border-gray-200">
                        ...
                        <div id="lesson-{{ $lessonIndex }}" class="lesson-container w-full">
                            @forelse($section->items as $contentIndex => $block)
                                <div id="content-{{ $lessonIndex }}-{{ $contentIndex }}" class="content-block w-full">
                                    ...
                                    @if($section->is_exam)
                                        <div class="text-center mb-6 w-full"><span
                                                class="inline-block px-4 py-1.5 bg-red-900 text-white text-[10px] font-black uppercase tracking-widest rounded-lg shadow-sm">Examination
                                                Section</span></div>
                                    @endif
                                    ...
                                    @if($hasMedia)
                                        ...
                                        @endphp
                                        <div
                                            class="mb-6 w-full {{ ($isQuizOrExam || $isTextContent) ? 'max-w-2xl mx-auto' : 'max-w-4xl mx-auto' }}">
                                            @if($isPdf)
                                                <div class="pdf-container media-container shadow-sm border border-gray-200"
                                                    data-pdf-url="{{ $mediaUrl }}" id="media-{{ $lessonIndex }}-{{ $contentIndex }}">
                                                    <div class="pdf-toolbar shrink-0">
                                                        <div class="flex items-center gap-4">
                                                            <span class="text-sm font-bold"><i class="fas fa-file-pdf mr-2"></i> PDF
                                                                Document (<span class="pdf-page-count">?</span> Pages)</span>
                                                        </div>
                                                        <div class="flex items-center gap-3">
                                                            <button onclick="pdfZoomOut('media-{{ $lessonIndex }}-{{ $contentIndex }}')"
                                                                class="hover:text-red-400 transition"><i
                                                                    class="fas fa-search-minus"></i></button>
                                                            <span class="pdf-scale text-xs font-bold w-12 text-center">100%</span>
                                                            <button onclick="pdfZoomIn('media-{{ $lessonIndex }}-{{ $contentIndex }}')"
                                                                class="hover:text-red-400 transition"><i
                                                            ...
                                                            <button
                                                                ...
                                                                class="fs-toggle-btn hover:text-red-400 transition ml-3 border-l border-gray-600 pl-3"
                                            ...
                                            @elseif($isVideo)
                                                <div class="video-wrapper shadow-xl border border-gray-800 media-container"
                                                    ...
                                                    <button
                                                        ...
                                                        class="fs-toggle-btn absolute top-4 right-4 bg-black/60 text-white w-10 h-10 flex items-center justify-center rounded-xl opacity-0 group-hover:opacity-100 transition hover:bg-red-900 shadow-lg z-20"
                                                        title="Full Screen"><i class="fas fa-expand"></i></button>
                                                </div>
                                            @elseif($isImage)
                                                <div class="rounded-2xl overflow-hidden bg-gray-200 border border-gray-200 flex justify-center w-full relative group media-container"
                                                    id="media-{{ $lessonIndex }}-{{ $contentIndex }}">
                                                    <img src="{{ $mediaUrl }}" class="object-contain max-h-[60vh] w-full">
                                                    <button
                                                        onclick="openMediaFullscreen('media-{{ $lessonIndex }}-{{ $contentIndex }}', 'image')"
                                                        class="fs-toggle-btn absolute top-4 right-4 bg-black/60 text-white w-10 h-10 flex items-center justify-center rounded-xl opacity-0 group-hover:opacity-100 transition hover:bg-red-900 shadow-lg"
                                                        title="Full Screen"><i class="fas fa-expand"></i></button>
                                                </div>
                                            @endif
                                        </div>
                                    ...
                                    @if($isTextContent)
                                        <div
                                            class="bg-white rounded-3xl p-6 md:p-8 shadow-sm border border-gray-100 prose prose-gray max-w-none text-gray-800 text-lg leading-relaxed mx-auto">
                                            {!! nl2br(e($block->question_text)) !!}
                                        </div>
                                    @elseif($isQuizOrExam)
                                        <div
                                            class="bg-white rounded-3xl p-6 md:p-8 shadow-sm border-2 border-blue-200 relative overflow-hidden mx-auto max-w-2xl">
                                            <div class="absolute top-0 left-0 w-1.5 h-full bg-blue-500"></div>
                                            <div class="flex justify-between items-start gap-4 mb-4">
                                                <h3 class="text-xl font-bold text-gray-900">{{ $block->question_text }}</h3>
                                                <span
                                                    class="px-2 py-1 bg-blue-50 text-blue-600 text-[10px] font-black uppercase tracking-widest rounded border border-blue-100 shrink-0">Answer
                                                    Key</span>
                                            </div>
                                            ...
                                            <div class="space-y-3">
                                                @if(in_array($block->type, ['mcq', 'true_false', 'checkbox']))
                                                    @foreach($block->options as $option)
                                                        <div
                                                            class="flex items-center p-4 rounded-xl border {{ $option->is_correct ? 'border-green-500 bg-green-50' : 'border-gray-200 bg-gray-50' }}">
                                                            <div
                                                                class="w-6 h-6 rounded-full flex items-center justify-center mr-3 shrink-0 {{ $option->is_correct ? 'bg-green-500 text-white' : 'bg-gray-200 text-gray-400' }}">
                                                                <i
                                                                    class="fas {{ $option->is_correct ? 'fa-check' : 'fa-times' }} text-xs"></i>
                                                            </div>
                                                            <span
                                                                class="text-base {{ $option->is_correct ? 'text-green-900 font-bold' : 'text-gray-500' }}">{{ $option->option_text }}</span>
                                                        </div>
                                                    @endforeach
                                                    ...
                                                    <div class="p-4 bg-gray-50 border border-gray-200 rounded-xl">
                                                        <div class="flex justify-between mb-2">
                                                            <h4 class="text-xs font-bold text-gray-500 uppercase tracking-widest">
                                                                Accepted Answers:</h4>
                                                            <span
                                                                class="text-[10px] font-bold {{ $block->is_case_sensitive ? 'text-red-500' : 'text-gray-400' }}">{{ $block->is_case_sensitive ? 'Case Sensitive' : 'Not Case Sensitive' }}</span>
                                                        </div>
                                                        <ul class="list-disc list-inside text-gray-800 ml-2 font-medium">
                                                            ...
                                                            @empty
                                                                <li class="text-gray-400 italic list-none text-sm">No specific auto-grade
                                                                    answers provided.</li>
                                                            @endforelse
                                                        </ul>
                                                    </div>
                            ...
                            @empty
                                <div
                                    class="content-block w-full active bg-white rounded-3xl p-12 shadow-sm border border-gray-100 text-center max-w-xl mx-auto">
            ...
            <div id="bottom-nav-bar"
                class="bg-white border-t border-gray-200 p-4 flex justify-between items-center shrink-0 w-full shadow-[0_-4px_6px_-1px_rgba(0,0,0,0.05)] z-20">
                <button type="button" id="btn-prev" onclick="navigateContent(-1)"
                    class="px-5 py-2.5 bg-gray-100 text-gray-600 font-bold rounded-xl hover:bg-gray-200 transition flex items-center gap-2"><i
                        class="fas fa-arrow-left"></i> Prev</button>
                <div class="text-center">
                    <span id="bottom-content-counter"
                        class="text-xs font-black text-gray-400 uppercase tracking-widest">Content ? of ?</span>
                </div>
                <button type="button" id="btn-next" onclick="navigateContent(1)"
                    class="px-5 py-2.5 bg-red-900 text-white font-bold rounded-xl hover:bg-red-950 transition shadow-md flex items-center gap-2">Next
        ...
        <aside id="evaluation-sidebar"
            class="w-80 lg:w-[400px] bg-white flex flex-col z-40 h-full shadow-2xl lg:shadow-none absolute right-0 lg:relative transform translate-x-full lg:translate-x-0 transition-transform duration-300">

            <div class="p-4 bg-red-900 text-white shrink-0 flex justify-between items-center shadow-md z-10">
                <div>
                    <h2 class="font-black text-lg flex items-center gap-2 truncate" title="{{ $criteriaTitle }}"><i
                            class="fas fa-clipboard-check text-red-200"></i> {{ Str::limit($criteriaTitle, 20) }}</h2>
                    <p class="text-[10px] text-red-200 uppercase tracking-widest font-bold mt-0.5">Required to Pass:
                        {{ $passingRate }}%</p>
                </div>
                <button onclick="toggleMobileEval()"
                    class="lg:hidden w-8 h-8 rounded-full bg-white/20 flex items-center justify-center hover:bg-white/30 transition"><i
            ...
            <div class="flex-1 overflow-y-auto sidebar-scroll p-4 bg-gray-50 border-l border-gray-200">
                ...
                <div class="bg-white p-3 rounded-xl border border-gray-200 shadow-sm mb-6">
                    <p class="text-[10px] font-black text-gray-400 uppercase tracking-widest mb-2 text-center">Rating
                        Guide</p>
                    <div class="flex justify-between text-center gap-1">
                        <div class="flex-1 bg-gray-50 rounded border border-gray-100 p-1"><span
                                class="block font-bold text-gray-800 text-sm">1</span><span
                                class="text-[9px] text-gray-500 font-medium">Poor</span></div>
                        <div class="flex-1 bg-gray-50 rounded border border-gray-100 p-1"><span
                                class="block font-bold text-gray-800 text-sm">2</span><span
                                class="text-[9px] text-gray-500 font-medium">Fair</span></div>
                        <div class="flex-1 bg-gray-50 rounded border border-gray-100 p-1"><span
                                class="block font-bold text-gray-800 text-sm">3</span><span
                                class="text-[9px] text-gray-500 font-medium">Avg</span></div>
                        <div class="flex-1 bg-gray-50 rounded border border-gray-100 p-1"><span
                                class="block font-bold text-gray-800 text-sm">4</span><span
                                class="text-[9px] text-gray-500 font-medium">Good</span></div>
                        <div class="flex-1 bg-gray-50 rounded border border-gray-100 p-1"><span
                                class="block font-bold text-gray-800 text-sm">5</span><span
                                class="text-[9px] text-gray-500 font-medium">Exc</span></div>
                    </div>
                </div>
                ...
                <form id="evaluation-form" class="space-y-6">
                    ...
                    @forelse($rubricData as $index => $category)
                        <div class="bg-white rounded-xl border border-gray-200 p-4 shadow-sm">
                            <h4
                                class="font-black text-gray-900 text-sm mb-4 uppercase tracking-wide border-b border-gray-100 pb-2">
                                        ...
                                        <div class="flex justify-between gap-1.5">
                                            @for($i = 1; $i <= 5; $i++)
                                                <label class="flex-1 eval-radio cursor-pointer text-center group">
                                                    <input type="radio" name="item_{{ $globalItemCounter }}" value="{{ $i }}"
                                                        class="hidden" onchange="calculateScore()">
                                                    <div
                                                        class="py-1.5 rounded border border-gray-200 bg-gray-50 text-gray-500 font-bold text-xs transition-all group-hover:bg-gray-100">
                                                        {{ $i }}</div>
                                                </label>
                                            @endfor
                                        </div>
                    ...
                    @empty
                        <div class="p-4 text-center text-sm text-gray-500 italic border border-gray-200 rounded-xl">No
                            global rubric configured.</div>
                    @endforelse

                    <div class="bg-white rounded-xl border border-gray-200 p-4 shadow-sm">
                        <label class="font-bold text-gray-900 text-sm mb-1 block">Admin Remarks & Feedback <span
                                class="text-red-500">*</span></label>
                        <p class="text-[10px] text-gray-500 mb-3 leading-tight">These comments will be sent to the
                            instructor if returned to draft.</p>
                        <textarea id="admin-remarks" rows="4"
                            class="w-full bg-gray-50 border border-gray-200 rounded-xl p-3 text-sm focus:ring-2 focus:ring-red-900 outline-none transition-all resize-none"
                            placeholder="Provide actionable feedback here..."></textarea>
                    </div>
                </form>
            </div>

            <div class="p-4 bg-white border-t border-gray-200 shrink-0 space-y-2 z-10">
                <button type="button" id="btn-approve" onclick="submitEvaluation('published')" disabled
                    class="w-full py-3 bg-green-600 text-white font-bold rounded-xl shadow-md hover:bg-green-700 transition disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2">
                    <i class="fas fa-check-circle"></i> Approve & Publish
                </button>
                <button type="button" id="btn-reject" onclick="submitEvaluation('draft')"
                    class="w-full py-3 bg-gray-100 text-gray-700 border border-gray-200 font-bold rounded-xl shadow-sm hover:bg-gray-200 hover:text-red-600 hover:border-red-200 transition flex items-center justify-center gap-2">
    ...
    <div id="fs-global-controls"
        ...
        <button onclick="closeMediaFullscreen()"
            class="absolute top-4 right-4 sm:top-15 sm:right-8 pointer-events-auto bg-black/60 hover:bg-red-600 text-white rounded-full w-12 h-12 flex items-center justify-center backdrop-blur transition-colors shadow-2xl border border-white/10"
        ...
        <div class="absolute bottom-8 left-1/2 -translate-x-1/2 flex items-center gap-6 pointer-events-auto">
            <button onclick="mediaFsNavigate(-1)"
                class="bg-black/60 hover:bg-red-900 text-white rounded-full h-14 w-14 flex items-center justify-center backdrop-blur transition-colors shadow-2xl border border-white/10"
                id="fs-btn-prev">
    ...
    {{-- SNACKBAR --}}
    <div id="eval-snackbar"
        class="fixed bg-gray-900 bottom-6 right-6 z-[9999] transform translate-y-24 opacity-0 transition-all duration-300 flex items-center gap-3 px-5 py-4 rounded-xl shadow-2xl font-medium text-sm text-white">
