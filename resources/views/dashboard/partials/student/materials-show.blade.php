...
<body class="bg-gray-50 font-sans text-gray-900 min-h-screen overflow-x-hidden selection:bg-[#a52a2a] selection:text-white">
            ...
            <div class="max-w-6xl mx-auto px-4 md:px-8 h-16 flex items-center justify-between">
                ...
                <div class="flex items-center gap-3 bg-gray-50 px-3 py-1.5 rounded-full border border-gray-100">
                    <div class="text-right hidden sm:block">
                        ...
                        <p class="text-[9px] text-[#a52a2a] uppercase font-black tracking-widest">{{ auth()->user()->role ?? 'Student' }}</p>
                    </div>
        ...
        <main class="flex-1 max-w-5xl mx-auto w-full py-8 px-4 sm:px-6 lg:px-8 pb-24">
            ...
            <div class="bg-white rounded-3xl shadow-sm border border-gray-100 p-6 md:p-8 mb-10 flex flex-col md:flex-row gap-8 lg:gap-12">
                ...
                <div class="w-full md:w-1/2 lg:w-7/12 flex flex-col">
                    ...
                    <div>
                        @if($isEnrolled)
                            <button class="w-full sm:w-auto px-8 py-3.5 bg-green-600 text-white font-bold rounded-xl shadow-lg shadow-green-600/20 flex items-center justify-center gap-2 cursor-default">
                                <i class="fas fa-check-circle text-lg"></i> Enrolled
                            </button>
                        @else
                            <button onclick="enrollInMaterial({{ $material->id }}, this)" class="w-full sm:w-auto px-8 py-3.5 bg-[#a52a2a] text-white font-bold rounded-xl hover:bg-red-800 transition shadow-lg shadow-[#a52a2a]/20 flex items-center justify-center gap-2">
            ...
            <div class="bg-white rounded-3xl shadow-sm border border-gray-100 p-2 md:p-4">
                ...
                @forelse($material->lessons as $index => $lesson)
                    <div class="p-4 rounded-2xl transition-colors border border-transparent flex items-start gap-4 group {{ $isEnrolled ? 'hover:bg-gray-50 hover:border-gray-100 cursor-pointer' : 'opacity-70 cursor-not-allowed' }}">
                        <div class="h-10 w-10 shrink-0 rounded-xl bg-gray-100 text-gray-400 font-black flex items-center justify-center {{ $isEnrolled ? 'group-hover:bg-[#a52a2a]/10 group-hover:text-[#a52a2a]' : '' }} transition-colors">
                            {{ $index + 1 }}
                        </div>
                        <div class="flex-1 min-w-0 pt-1.5">
                            <h4 class="font-bold text-gray-900 {{ $isEnrolled ? 'group-hover:text-[#a52a2a]' : '' }} transition-colors text-lg">{{ $lesson->title }}</h4>
    ...
    <script>
        ...
        function navigateBack() {
            ...
            // Remove the slide-in class and trigger the slide-out class
            wrapper.classList.remove('animate-slide-in');
            wrapper.classList.add('animate-slide-out');
            
            // Wait 300ms for the animation to finish, then go back in history
            setTimeout(() => {
                window.history.back();
            }, 300);
        }
