...
<head>
    ...
    <title>Certificate {{ auth()->check() && auth()->id() === $enrollment->user_id ? 'Achieved' : 'Verification' }} -
        {{ $enrollment->material->title }}</title>
...
<body class="bg-gray-50 font-sans text-gray-900 min-h-screen selection:bg-[#a52a2a] selection:text-white">
    ...
    <div class="max-w-6xl mx-auto py-12 px-4 sm:px-6 relative min-h-screen flex flex-col items-center justify-center">
        ...
        <div
            class="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[600px] h-[600px] {{ $isOwner ? 'bg-yellow-400/10' : 'bg-green-400/10' }} rounded-full blur-3xl pointer-events-none">
        </div>
        ...
        <div class="text-center mb-10 relative z-10 animate-float-in w-full max-w-2xl mx-auto">
            ...
            {{-- LMS Logo --}}
            <img src="{{ asset('storage/images/lms-logo-red.png') }}" alt="LMS Logo"
                class="h-14 sm:h-16 mx-auto mb-8 object-contain">
            ...
            @else
                ...
                <div
                    class="w-24 h-24 bg-green-100 rounded-full flex items-center justify-center mx-auto mb-6 shadow-2xl shadow-green-500/20 border-4 border-white">
                    <i class="fas fa-check-circle text-5xl text-green-500"></i>
                </div>
                <h1 class="text-3xl md:text-4xl font-black text-gray-900 mb-4 tracking-tight">Official Certificate Record
                </h1>
                <p class="text-lg text-gray-600">This page serves as official verification that <strong
                        class="text-gray-900">{{ $enrollment->user->first_name }}
                        {{ $enrollment->user->last_name }}</strong> successfully completed the learning module <span
                        class="font-bold text-[#a52a2a]">"{{ $enrollment->material->title }}"</span>.</p>
            @endif
        ...
        <div
            class="w-full max-w-5xl bg-white p-4 sm:p-8 rounded-3xl shadow-2xl border border-gray-100 relative z-10 mb-10 mx-auto overflow-hidden flex justify-center">
                ...
                <div class="cert-content relative bg-white shrink-0 origin-top flex flex-col items-center">
                    ...
                    <div
                        ...
                        <div class="w-full h-[150px] flex items-center justify-center mb-2">
                            <img src="{{ asset('images/lms-cert-header.png') }}" class="h-full w-auto object-contain"
                                alt="Header">
                        </div>
                            ...
                            <tr>
                                ...
                                <td class="w-1/3 align-bottom text-center">
                                    <div class="inline-flex flex-col items-center">
                                        <div
                                            class="w-[110px] h-[110px] bg-[#d97706] rounded-full border-[8px] border-[#fde68a] flex items-center justify-center text-white text-[55px] shadow-inner mb-2 leading-none pb-2">
                                            ★
                                        </div>
                                        <div class="text-[12px] text-[#555] font-bold uppercase tracking-widest mt-1">
                                            Official Award</div>
                                    </div>
                                </td>
...
<a href="{{ route('student.certificate.download', ['hashid' => $hashid]) }}" target="_blank"
