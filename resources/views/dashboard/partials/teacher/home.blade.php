<div class="mb-8 flex flex-col md:flex-row md:items-center md:justify-between gap-4">
    ...
    <div class="flex items-center gap-3">
        <button onclick="loadPartial('{{ route('dashboard.home') }}', document.getElementById('nav-home-btn'))"
            class="p-2.5 bg-white border border-gray-200 rounded-xl text-gray-600 hover:bg-gray-50 transition shadow-sm" title="Refresh Data">
...
<div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
    <div class="bg-gradient-to-r from-amber-500 to-orange-500 rounded-2xl p-6 text-white shadow-lg shadow-orange-900/20 relative overflow-hidden">
        <div class="relative z-10">
            <p class="text-orange-100 text-xs font-bold uppercase tracking-wider mb-1 flex items-center gap-1.5"><i class="fas fa-star"></i> Most Popular Module</p>
            @if($topModule)
                <h3 class="text-2xl font-black mb-1 line-clamp-1" title="{{ $topModule->title }}">{{ $topModule->title }}</h3>
    ...
    <div class="bg-gradient-to-r from-blue-600 to-indigo-700 rounded-2xl p-6 text-white shadow-lg shadow-blue-900/20 flex items-center justify-between">
        <div>
    ...
    <div class="bg-gradient-to-r from-emerald-500 to-green-600 rounded-2xl p-6 text-white shadow-lg shadow-green-900/20 flex items-center justify-between">
        <div>
...
<div class="grid grid-cols-1 lg:grid-cols-3 gap-8 pb-10">
    ...
    <div class="space-y-8">
                ...
                <div class="divide-y divide-gray-50">
                    @forelse($activeStudentsList as $enrollment)
                        <div class="p-4 hover:bg-gray-50 transition flex items-center gap-3">
                            <div class="w-8 h-8 rounded-full bg-blue-50 text-blue-600 flex items-center justify-center text-xs font-bold border border-blue-100 shrink-0">
                                {{ substr($enrollment->user->first_name, 0, 1) }}{{ substr($enrollment->user->last_name, 0, 1) }}
                            </div>
                            <div class="flex-1 min-w-0">
                                <p class="text-sm font-bold text-gray-900 truncate">{{ $enrollment->user->first_name }} {{ $enrollment->user->last_name }}</p>
                                <p class="text-[10px] text-gray-500 truncate" title="{{ $enrollment->material->title }}">
                                    Studying: <span class="font-medium text-gray-700">{{ $enrollment->material->title }}</span>
                                </p>
                            </div>
                            <span class="text-[10px] text-gray-400 whitespace-nowrap">{{ $enrollment->updated_at->diffForHumans(null, true, true) }}</span>
                        </div>
                    @empty
        ...
        <div class="bg-white rounded-2xl border-2 {{ $pendingInvitesCount > 0 ? 'border-dashed border-amber-200' : 'border-solid border-gray-100' }} shadow-sm overflow-hidden relative">
            @if($pendingInvitesCount > 0)
                <div class="absolute top-0 right-0 w-16 h-16 bg-amber-50 rounded-bl-full flex items-start justify-end p-3 pointer-events-none">
            ...
            <div class="p-5 border-b border-gray-50">
                <h3 class="font-bold text-gray-900 flex items-center gap-2">
                    Pending Invitations <span class="bg-gray-100 text-gray-600 px-2 py-0.5 rounded-full text-xs">{{ $pendingInvitesCount }}</span>
                </h3>
                ...
                @forelse($pendingInvitesList as $invite)
                    <div class="p-4 hover:bg-gray-50 transition">
                        <div class="flex justify-between items-start mb-1">
                            ...
                            <span class="text-[10px] text-amber-500 font-bold bg-amber-50 px-2 py-0.5 rounded border border-amber-100">Waiting</span>
                        </div>
                        <p class="text-[10px] text-gray-500 flex items-center gap-1.5 truncate">
                            <i class="fas fa-book-open text-gray-400"></i> Module: 
                            <span class="font-medium text-gray-700">{{ $invite->material->title ?? 'Unknown Module' }}</span>
                        </p>
                    </div>
                @empty
...
<script>
    // 1. Line Chart
    new Chart(document.getElementById('teacherActivityChart').getContext('2d'), {
        type: 'line',
        data: {
    ...
    // 2. Doughnut Chart
    if(document.getElementById('teacherMaterialsChart')) {
        new Chart(document.getElementById('teacherMaterialsChart').getContext('2d'), {
            type: 'doughnut',
            data: {
