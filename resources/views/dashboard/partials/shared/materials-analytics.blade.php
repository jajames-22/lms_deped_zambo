    ...
    <div class="space-y-12 max-w-7xl mx-auto">
        ...
        <section id="class-overview" class="scroll-mt-20">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                ...
                {{-- Passing Rate Card --}}
                @php
                    $passBg = 'bg-gray-100 border-gray-300 border-l-gray-400';
                    $passText = 'text-gray-600';

                    if (isset($passRate)) {
                        if ($passRate >= 75) {
                            $passBg = 'bg-green-50 border-green-200 border-l-green-500';
                            $passText = 'text-green-600';
                        } elseif ($passRate >= 50) {
                            $passBg = 'bg-amber-50 border-amber-200 border-l-amber-500';
                            $passText = 'text-amber-600';
                        } else {
                            $passBg = 'bg-red-50 border-red-200 border-l-red-500';
                            $passText = 'text-red-600';
                        }
                    }
                @endphp
                ...
                <div class="p-5 rounded-2xl shadow-sm border border-l-4 {{ $passBg }}">
                    ...
                    @else
                        <p class="text-lg font-bold text-gray-400 mt-2 italic">
                            N/A
                        </p>
                    @endif
                </div>
                                    ...
                                    <tr class="competency-row hover:bg-gray-50/50 transition">
                                        ...
                                        @if($cat->has_quiz)
                                            <td class="px-6 py-4 text-center">
                                                ...
                                                @else
                                                    <span
                                                        class="px-2.5 py-1 rounded-md text-xs font-bold {{ $cat->mps >= 75 ? 'text-green-700 bg-green-100' : ($cat->mps <= 40 ? 'text-red-700 bg-red-100' : 'text-amber-700 bg-amber-100') }}">{{ $cat->mps }}%</span>
                                                @endif
                                            </td>
                                            <td class="px-6 py-4 text-center">
                                                @if(($cat->total_answers ?? 0) == 0)
                    ...
                    <div
                        ...
                        <div>
                            ...
                            <p class="text-xs text-white/70 flex items-center gap-1">
                                Metrics based on {{ $quizItemsCount }} Quiz item(s) and {{ $examItemsCount }} Exam
                                item(s).
                            </p>
                        </div>

                        {{-- Overall Student Average Block --}}
                        <div class="flex items-center gap-4 bg-black/20 px-4 py-2 rounded-xl">
                            <div class="text-right">
                                <p class="text-[10px] uppercase tracking-wider text-white/70 font-bold mb-0.5">Overall
                                    Student Avg</p>
                                <p class="text-2xl font-black leading-none">
                                    {{ $hasQuizzes || $hasExams ? $overallAverage . '%' : 'N/A' }}</p>
                            </div>
                            <i class="fas fa-medal text-4xl text-yellow-400 opacity-90 drop-shadow-md"></i>
                        </div>
                    ...
                    <div id="student-table-wrapper" class="flex-1 flex flex-col">
                        <table class="w-full text-left text-sm text-gray-600">
                            ...
                            <tbody class="divide-y divide-gray-100">
                                ...
                                @empty
                                    <tr>
                                        <td colspan="6" class="px-4 py-8 text-center text-gray-400">No student enrollments
                                            yet.</td>
                                    </tr>
                                @endforelse
                            </tbody>
        ...
        <section id="item-analysis" class="scroll-mt-20 mt-8">
            ...
            @if(!$hasQuizzes && !$hasExams)
                <div
                    class="flex flex-col items-center justify-center p-12 text-center bg-gray-50/50 rounded-2xl border-2 border-gray-200 border-dashed">
                    <div
                        class="h-16 w-16 bg-gray-200 text-gray-400 rounded-full flex items-center justify-center text-2xl mb-4">
                                    ...
                                    <tbody class="divide-y divide-gray-100" id="quizItemTableBody">
                                        @foreach($quizItemAnalysis ?? [] as $index => $item)
                                            ...
                                            <tr class="hover:bg-gray-50/50 transition quiz-item-row">
                                                ...
                                                <td class="px-6 py-4 text-center">
                                                    <div class="flex flex-col items-center">
                                                        @if($totalItemAnswers == 0)
                                                            ...
                                                            <span
                                                                class="text-[10px] text-gray-400 font-bold tracking-wider uppercase"><i
                                                                    class="fas fa-hourglass-half"></i> No Data</span>
                                                        @else
                                                            <span
                                                            ...
                                                            <div class="flex gap-2 text-[10px] font-bold text-gray-400 mb-1">
                                                                <span class="text-green-600"><i class="fas fa-check"></i>
                                                                    {{ $item->correct_count }}</span>
                                                                <span class="text-red-500"><i class="fas fa-times"></i>
                                                                    {{ $item->wrong_count }}</span>
                                                            </div>
                                                ...
                                                <td class="px-6 py-4">
                                                    @if($totalItemAnswers == 0)
                                                        <div class="flex items-center gap-2 text-gray-400 text-xs italic">Waiting for
                                                            student responses...</div>
                                                    @else
        ...
        <form action="{{ route('dashboard.materials.export', $material->id) }}" method="GET" target="_blank">
            <div class="space-y-3 mb-6">
                <label
                    ...
                    <input type="checkbox" name="check_metrics" checked
                ...
                <label
                    class="flex items-center gap-3 p-3 border border-gray-100 rounded-xl cursor-pointer hover:bg-gray-50">
                    <input type="checkbox" name="check_competency" checked
                        class="w-5 h-5 text-[#a52a2a] rounded border-gray-300 focus:ring-[#a52a2a]">
            ...
            <div class="flex gap-3">
                <button type="submit" name="action" value="print" onclick="setTimeout(toggleExportModal, 500)"
                    class="flex-1 bg-gray-100 hover:bg-gray-200 text-gray-800 py-2.5 rounded-xl font-medium flex items-center justify-center gap-2 border-0 cursor-pointer transition">
                    <i class="fas fa-print"></i> Print
                </button>
                <button type="submit" name="action" value="pdf" onclick="setTimeout(toggleExportModal, 500)"
                    class="flex-1 bg-gray-800 hover:bg-gray-900 text-white py-2.5 rounded-xl font-medium flex items-center justify-center gap-2 border-0 cursor-pointer transition shadow-md">
