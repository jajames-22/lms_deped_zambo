                                ...
                                <tr>
                                    <th class="px-4 md:px-6 py-4">
                                        <div class="flex items-center gap-3">
                                            <input type="checkbox" id="selectAllAccessCheckbox"
                                                class="hidden rounded border-gray-300 text-[#a52a2a] focus:ring-[#a52a2a] cursor-pointer w-4 h-4 mt-0.5">
                                            <span>Student</span>
                                        </div>
                                    </th>
                                    ...
                                    <tr class="hover:bg-gray-50/70 transition align-top">
                                        ...
                                        <td class="px-4 md:px-6 py-4">
                                            <div class="flex items-start gap-3 min-w-0">
                                                ...
                                                <input type="checkbox" value="{{ $access->id }}"
                                                    data-status="{{ $access->status }}"
                                                    class="access-checkbox hidden rounded border-gray-300 text-[#a52a2a] focus:ring-[#a52a2a] cursor-pointer w-4 h-4 mt-2.5 transition-all">
                                                ...
                                                <div class="min-w-0 flex-1">
                                                    @php
                                                        $retakesCount = $access->retakes ?? 0;
                                                        $retakesUsed = min(3, $retakesCount);
                                                        $status = $access->current_enrollment ? $access->current_enrollment->status : null;
                                                        if ($retakesCount >= 3 && $status !== 'completed' && $status !== 'in_progress') {
                                                            $status = 'failed';
                                                        }
                                                        $showResults = in_array($status, ['completed', 'failed']) || ($status === 'dropped' && (!is_null($access->current_enrollment->completed_at) || ($access->current_enrollment->progress_percentage ?? 0) >= 100 || $retakesCount > 0));
                                                        ...
                                                        <div class="md:hidden mt-3">
                                                            <div class="flex items-center justify-between text-xs mb-1.5">
                                                                <span class="font-bold text-gray-700">
                                                                    {{ ucwords(str_replace('_', ' ', $status ?? $access->status)) }}
                                                                </span>
                                        ...
                                        <td class="px-4 py-4 whitespace-nowrap">
                                            @if($status === 'failed')
                                                <span
                                                    class="px-2.5 py-1 bg-red-50 text-red-700 text-[10px] font-black uppercase tracking-wider rounded-md border border-red-100">Failed</span>
                                            @elseif($status === 'completed')
                                                <span
                                                    class="px-2.5 py-1 bg-green-50 text-green-700 text-[10px] font-black uppercase tracking-wider rounded-md border border-green-100">Completed</span>
                                            @elseif($access->status === 'enrolled' || $status === 'in_progress')
                                                <span
                                            ...
                                            @elseif($access->status === 'dropped')
                                                <span
                                                    class="px-2.5 py-1 bg-amber-50 text-amber-700 text-[10px] font-black uppercase tracking-wider rounded-md border border-amber-100">Dropped</span>
                                            @else
                                                <span
                                                    class="px-2.5 py-1 bg-gray-50 text-gray-700 text-[10px] font-black uppercase tracking-wider rounded-md border border-gray-100">{{ ucwords($access->status) }}</span>
                                            @endif
                                        </td>
                                        ...
                                        <td class="px-4 py-4 hidden md:table-cell">
                                            @if($access->current_enrollment)
                                                <div class="flex flex-col min-w-[140px]">
                                                    ...
                                                    <span
                                                        class="font-bold {{ $status === 'failed' ? 'text-red-600' : 'text-gray-700' }} text-xs!">
                                        ...
                                        <td class="px-4 py-4 text-center hidden sm:table-cell">
                                            <span
                                                class="inline-flex items-center justify-center min-w-[44px] px-2.5 py-1 rounded-md {{ $retakesUsed > 0 ? 'bg-purple-50 text-purple-700 border border-purple-100' : 'bg-gray-100 text-gray-600 border border-gray-200' }} font-bold text-xs">
                                                {{ $retakesUsed }}/3
                                            </span>
                                        </td>
                        ...
                        <div class="flex justify-between items-start">
                            ...
                            @if(($material->items_count ?? 0) > 0 || ($material->lessons_count ?? 0) > 0)
                                <div class="relative group cursor-help">
                                    <i
                                        class="fas fa-info-circle text-gray-400 transition-colors group-hover:text-[#a52a2a] text-[11px]"></i>
                                    <div
                                        class="absolute right-0 bottom-full mb-2 w-80 bg-white rounded-xl shadow-xl opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-200 z-[100] border border-gray-100 text-left font-normal normal-case tracking-normal">
                                        <div
                                            class="p-3 max-h-60 overflow-y-auto custom-scrollbar text-gray-700 text-[11px] leading-relaxed">
                                            <strong class="text-gray-900 block mb-1">Passing Rate Formula</strong>
                                            <p
                                                class="m-0 mb-2 font-mono text-[10px] bg-gray-50 p-1.5 rounded text-gray-600 border border-gray-100">
                                                Pass Rate = (Passed Students &divide; Evaluated Students) &times; 100</p>

                                            <div class="space-y-1 mb-2 border-t border-gray-100 pt-2">
                                                <div><strong class="text-gray-900">Passed Students:</strong> Students whose
                                                    score is equal to or greater than the passing score
                                                    ({{ $passingScore ?? 80 }}%).</div>
                                                <div><strong class="text-gray-900">Evaluated Students:</strong> Students who
                                                    have completed the assessment and received a score.</div>
                                            </div>

                                            <div class="bg-gray-50 p-2.5 rounded border border-gray-100 mt-2">
                                                <strong
                                                    class="text-gray-900 block mb-1 border-b border-gray-200 pb-1">Current
                                                    Computation</strong>
                                                @if(($evaluatedCount ?? 0) > 0)
                                                    <div class="grid grid-cols-2 gap-1 my-1.5 text-[10px]">
                                                        <div>Passing Score: <strong
                                                                class="text-gray-900">{{ $passingScore ?? 80 }}</strong></div>
                                                        <div>Students Evaluated: <strong
                                                                class="text-gray-900">{{ $evaluatedCount ?? 0 }}</strong></div>
                                                        <div class="col-span-2">Students Who Passed: <strong
                                                                class="text-gray-900">{{ $passCount ?? 0 }}</strong></div>
                                                    </div>
                                                    <div class="pt-1.5 border-t border-gray-200">
                                                        <p class="text-[10px] text-gray-600 m-0">Formula: ({{ $passCount ?? 0 }}
                                                            &divide; {{ $evaluatedCount ?? 0 }}) &times; 100</p>
                                                        <p class="font-bold text-gray-900 mt-0.5 mb-0">Result: {{ $passRate }}%
                                                        </p>
                                                    </div>
                                                @else
                                                    <p class="text-gray-500 italic text-[10px] m-0 py-1">No evaluated students
                                                        yet.<br>Pass Rate cannot be calculated.</p>
                                                @endif
                                            </div>
                                        </div>
                                        <div
                                            class="absolute top-full right-2 w-0 h-0 border-x-[6px] border-x-transparent border-t-[8px] border-t-white">
                                        </div>
                                    </div>
                                </div>
                            @endif
...
<div id="studentActivityModal"
    class="fixed inset-0 z-[9999] hidden opacity-0 transition-opacity duration-300 flex items-center justify-center p-4">
    <div class="absolute inset-0 bg-gray-900/60" onclick="closeModal('studentActivityModal', 'studentActivityBox')">
    </div>
    <div id="studentActivityBox"
        ...
        <div class="shrink-0 p-8 pb-6 border-b border-gray-100 bg-white relative z-20">
            ...
            <p id="studentActivityDescription" class="text-gray-500 text-sm leading-relaxed mb-0">This module contains
                active student data. Choose how to handle existing records before reverting to Draft.</p>
        </div>
        ...
        <div class="flex-1 overflow-y-auto p-8 py-6">
            ...
            <div class="mb-6 p-4 bg-blue-50 border border-blue-100 rounded-xl">
                ...
                <p class="text-xs text-blue-700 mb-3">You may export the current student records before reverting this
                    module to Draft.</p>
                <button type="button"
                    onclick="closeModal('studentActivityModal', 'studentActivityBox'); setTimeout(() => openModal('exportListModal', 'exportListBox'), 300);"
                    class="px-4 py-2 bg-blue-600 text-white text-xs font-bold rounded-lg hover:bg-blue-700 transition shadow-sm">
            ...
            <div class="space-y-4 mb-6">
                <label
                    class="flex items-start gap-3 p-4 border border-gray-200 rounded-xl cursor-pointer hover:bg-gray-50 transition-colors">
                    <input type="radio" name="revert_data_option" value="keep" checked
                    ...
                    <div>
                        ...
                        <span class="block text-xs text-gray-500 mt-1">Students will remain enrolled and all progress,
                            scores, attempts, and submitted answers will be preserved. The module will simply become
                            unavailable to students while it is in Draft status.</span>
                    </div>
                </label>

                <label
                    class="flex items-start gap-3 p-4 border border-red-200 bg-red-50/50 rounded-xl cursor-pointer hover:bg-red-50 transition-colors">
                    <input type="radio" name="revert_data_option" value="clear"
                        class="mt-1 text-red-600 focus:ring-red-600">
                    <div>
                        <span class="block text-sm font-bold text-red-800">Clear Student Progress and Assessment
                            Results</span>
                        <span class="block text-xs text-red-600 mt-1">Students will remain enrolled, but all progress,
                            completion records, scores, quiz attempts, exam attempts, and submitted answers related to
                            this module will be permanently removed. Students will start from the beginning if the
                            module is republished.</span>
                        <span class="block text-xs font-bold text-red-600 mt-2"><i
                                class="fas fa-exclamation-triangle mr-1"></i> This action cannot be undone.</span>
                    </div>
                </label>
            </div>
        ...
        <div class="shrink-0 p-8 pt-6 border-t border-gray-100 bg-white relative z-20">
            <div class="flex gap-3">
                ...
                <button type="button" onclick="executeActivityRevertToDraft()" id="executeActivityRevertBtn"
                    class="flex-1 py-3 bg-amber-500 text-white font-bold rounded-xl shadow-lg shadow-amber-500/20 hover:bg-amber-600 transition flex justify-center items-center">Proceed
                    & Revert</button>
            </div>
        </div>
    </div>
                ...
                <label class="flex items-start gap-3 cursor-pointer" id="export-summary-label">
                    ...
                    <div id="card-summary"
                        ...
                        <div class="flex items-center gap-2 mb-1">
                            <i class="fas fa-file-excel text-[#a52a2a] text-sm"></i>
                            <span class="text-sm font-bold text-gray-800">Summary Report</span>
                            <span
                                class="text-[10px] bg-gray-100 text-gray-500 font-bold px-2 py-0.5 rounded-full uppercase">.XLSX</span>
                        </div>
...
<script>
    ...
    window.updateBulkAccessUI = function () {
        ...
        if (deleteBtn) {
            ...
            if (count === 0) deleteBtn.classList.add('opacity-50', 'cursor-not-allowed');
    ...
    window.dropAccess = function (accessId, btnElement) {
        // dropAccess is no longer used; kept as no-op to avoid JS errors from cached pages
    }
    ...
    window.executeDeleteEnrollment = function () {
        submitDeleteEnrollment();
    };

    window.submitDeleteEnrollment = function () {
...
<div id="deleteEnrollmentModal"
    ...
    <div id="deleteEnrollmentBox"
        ...
        <div class="px-6 py-5 border-b border-gray-100 flex items-center justify-between bg-red-50/50">
            ...
            <button onclick="closeModal('deleteEnrollmentModal', 'deleteEnrollmentBox')"
        ...
        <div class="p-6">
            ...
            <p class="text-sm text-gray-600 mb-4">
                The student will no longer have access to this module.
            </p>
            ...
            <div class="flex gap-3 rounded-xl border border-red-200 bg-red-50 p-4">
                ...
                <div class="text-sm">
                    ...
                    <p class="text-red-700">
                        Removing this student will permanently delete their enrollment, quiz answers, exam answers, and
                        all learning records for this module.
                    </p>
                </div>
            </div>
        </div>
        <div class="px-6 py-4 border-t border-gray-100 bg-gray-50 flex justify-end gap-3">
            ...
            <button onclick="executeDeleteEnrollment()" id="deleteEnrollmentSubmitBtn"
                ...
                <i class="fas fa-user-minus"></i> Remove from List
            </button>
        </div>
    </div>
</div>

{{-- Bulk Remove from List Confirmation Modal --}}
<div id="bulkDeleteEnrollmentModal"
    class="fixed inset-0 z-[9999] hidden opacity-0 transition-opacity duration-300 flex items-center justify-center p-4 sm:p-6"
    style="background-color: rgba(0, 0, 0, 0.5); backdrop-filter: blur(4px);">
    <div id="bulkDeleteEnrollmentBox"
        class="bg-white rounded-2xl shadow-xl w-full max-w-md transform transition-all duration-300 scale-95 overflow-hidden">
        <div class="px-6 py-5 border-b border-gray-100 flex items-center justify-between bg-red-50/50">
            <h3 class="text-lg font-black text-gray-900 flex items-center gap-2">
                <div class="w-8 h-8 rounded-lg bg-red-100 text-red-600 flex items-center justify-center">
            ...
            <button onclick="closeModal('bulkDeleteEnrollmentModal', 'bulkDeleteEnrollmentBox')"
                class="text-gray-400 hover:text-gray-600 transition p-2 rounded-lg hover:bg-gray-100">
                <i class="fas fa-times"></i>
            </button>
        </div>
        <div class="p-6">
            ...
            <div class="flex gap-3 rounded-xl border border-red-200 bg-red-50 p-4">
                ...
                <div class="text-sm">
                    ...
                    <p class="text-red-700">
                        Selected students' enrollments will be permanently deleted, including their quiz answers, exam
                        answers, and all learning records for this module.
                    </p>
                </div>
            </div>
        </div>
